...
users-own [
  ;;
  ; Where the user currently is: string in {
  ;   "etoile": waiting at etoile
  ;   "rer": travelling in a rer
  ;   ALTMODE: travelling in an alternative mode {"m1", "bus", "bike", "taxi", "walking"}
  ;    (arrived users are directly killed)
  ;  }
  user:status
  ...
  ;;
  ; congestion in the mode when the user departs from the origin station: for all modes corresponds to the effective congestion
